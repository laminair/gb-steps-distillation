...
def template_renders_documents(tok) -> bool:
    ...
    rec = [{"role": "user", "content": "summarise"},
           {"role": "assistant", "content": "ok"}]
    try:
        out = tok.apply_chat_template(
            rec, tokenize=False, add_generation_prompt=False,
...
def expectation(args: argparse.Namespace, identity: str) -> dict:
    ...
    return {
        ...
        "policies": {
            ...
            "explode_max_per_conv": (args.explode_max_per_conv
                                     if args.explode_assistant_turns else None),
            ...
            "max_completion_length": (int(args.max_completion_length)
                                      if prompt_budget_of(args) is not None else None),
            "prompt_budget": prompt_budget_of(args),
        },
        "seed": args.seed,
        "eval_fraction": args.eval_fraction,
        "max_examples": args.max_examples,
        "shard": {"index": int(getattr(args, "shard_index", 0) or 0),
        ...
        with path.open() as fh:
            for lineno, line in enumerate(fh, 1):
                ...
                try:
                    ...
                    raise PrepError(f"{dataset}:{lineno} is not valid JSON: {exc}") from exc
        return
    if path.exists():
        raise PrepError(f"--dataset {dataset} exists but is not a file. "
...
def load_tokenizer(tokenizer_dir: Path):
    ...
    if not tok_file.is_file():
        raise PrepError(f"{tok_file} is missing. --tokenizer must be a directory holding a "
...
def normalise(record: dict, *, think_policy: str, boundary: str,
              min_messages: int, stats: dict | None = None,
              documents_policy: str = "keep") -> tuple[dict | None, str]:
...
def explode_assistant_turns(rec: dict, *, max_per_conv: int, seed_key: str) -> list[dict]:
...
def prompt_token_count(record: dict, tok) -> int:
    ...
    objection is recorded at the --emit-row-id comment below: a predicate re-implemented here
    ...
    on a different rule is worse than one that makes no claim. Three things pay for it:
      ...
      2. Every kwarg here is the trainer's, including per-row `render_thinking` (the trainer's
         ...
         Arrow string column, this reads it before _emit() re-serialises it, so both branches
...
def measure(record: dict, tok, *, max_length: int, length_policy: str,
            boundary: str = "all_assistant",
            prompt_budget: int | None = None,
            detail: dict | None = None,
            totals: dict | None = None) -> tuple[dict | None, str, int, int]:
    ...
    except Exception as exc:  # template errors are data-dependent, not programmer errors
    ...
    if prompt_budget is not None:
        try:
            ...
            if detail is not None:
                detail.setdefault(f"prompt_template_error:{type(exc).__name__}", str(exc)[:600])
            return None, f"prompt_template_error:{type(exc).__name__}", n_tokens, n_target
        if totals is not None:
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.documents_policy == "drop" and renders_documents:
        print(f"WARNING: --documents-policy drop, but {tok_dir}'s chat template DOES render "
    ...
    if args.documents_policy == "keep" and not renders_documents:
        print("WARNING: --documents-policy keep, and this template DISCARDS `documents`. "
    ...
    # per-datapoint training manifest; corpus_manifest.json stays aggregate. Two files rather than one because a
    ...
    def _emit(rec, rid, explode_index, reason, n_tok, n_tgt):
        ...
        entry = {"src_id": rid, "out_id": oid, "disposition": "kept",
                 "reason": reason or "", "tokens": n_tok, "target_tokens": n_tgt,
                 "kept_index": len(kept)}
    ...
    g_in = 0
    for raw in load_records(args.dataset, args.dataset_split, args.dataset_config,
                            args.hf_home):
        if shard_count > 1:
        ...
        n_in += 1
        rid = row_id(raw)
        rec, reason = normalise(raw, think_policy=args.think_policy,
                               boundary=args.completion_boundary,
                               min_messages=args.min_messages, stats=notes,
                               documents_policy=args.documents_policy)
        if rec is None:
        ...
        for v_i, rec in enumerate(variants):
            ...
            if exploded:
                ...
                rec, v_reason = normalise(
                    rec, think_policy=args.think_policy,
                    boundary=args.completion_boundary,
                    min_messages=args.min_messages, stats=notes,
                    documents_policy=args.documents_policy)
                if rec is None:
                    ...
                    rows.append({"src_id": rid, "explode_index": v_i,
                                 "disposition": "dropped", "reason": key})
                    continue
            rec, reason, n_tok, n_tgt = measure(rec, tok, max_length=args.max_length,
                                                length_policy=args.length_policy,
                                                boundary=args.completion_boundary,
    ...
    if not kept:
        ...
        advice = ("Check --max-length, --think-policy and --completion-boundary against the "
        ...
        if drops.get("over_prompt_budget"):
            advice = (
                ...
                f"{args.max_completion_length} leaves {prompt_budget} tokens for the prompt, and "
                f"every prompt rendered longer. Widening --max-length raises that allowance; "
                f"lowering --max-completion-length raises it too, at the cost of a shorter "
                f"generation. " + advice)
        raise PrepError(
    ...
    manifest = {
        ...
        "tokenizer_identity": identity,
        # THE KEY IT COMPARES WITH, added 2026-09-21. `tokenizer_identity` above is whatever
        # read() preferred, which is a NAME when the tokenizer directory carries an identity
        # file and a HASH when it does not -- two different kinds of answer, and the guard
        # compares a corpus's answer against a student's. distill-tokenizer-align began writing
        # identity files after the deliverable corpora were built, so the bullet-4 pair arrived
        # with a name on one side and a hash on the other and the guard REFUSED a byte-identical
        # tokenizer. Recording both makes the corpus self-describing, so render_common's
        # comparison never has to fall back to re-hashing tokenizer_path -- a path that may be
        # gone by then, or worse, still there and since rewritten. NOT added to expectation():
        # that dict is the step_state fingerprint, and growing it would turn every existing
        # corpus's SKIP into a REFUSE, which is the failure corpus-render.sh:44 records.
        "tokenizer_sha256": tokenizer_identity.hash_tokenizer(tok_dir),
        ...
        "token_stats": {
            ...
            **({"prompt_tokens": _length_summary(prompt_lengths)} if prompt_lengths else {}),
        },
        "splits": splits,
    }
    ...
    manifest["rows"] = {
        ...
        "training_rows": sum(1 for r in rows
                             if r["disposition"] == "kept" and r.get("split") == "train"),
    }
    ...
    # nothing.
    #
    # Comparing against a bare n_in here was a real defect, not a conservative bound: it made
    # EVERY --explode-assistant-turns build raise on an otherwise complete corpus, at the very
    # end, after all the tokenizer work. It survived because the explosion tests exercised
    # explode_assistant_turns() directly and never called build() with the flag on.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--dataset-split", default="train", help="HF split (ignored for a file)")
    ...
    p.add_argument("--max-completion-length", type=int, default=0,
                   help="the trainer's max_completion_length (0 = no prompt filter). Drops "
                        ...
                        "`over_prompt_budget`.")
    p.add_argument("--length-policy", choices=LENGTH_POLICIES, default="drop",
    ...
    p.add_argument("--think-policy", choices=THINK_POLICIES, default="keep",
    ...
    p.add_argument("--completion-boundary", choices=BOUNDARIES, default="last_message",
    ...
    p.add_argument("--explode-max-per-conv", type=int, default=2,
    ...
    p.add_argument("--emit-row-id", action=argparse.BooleanOptionalAction, default=False,
    ...
    p.add_argument("--documents-policy", choices=DOCUMENTS_POLICIES, default="drop",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"  kept {c['kept']} of {c['input']} "
    ...
    print(f"  tokens: mean {t['mean_tokens']} max {t['max_tokens']} "
    ...
    if unused > 0:
        ...
        print(f"  NOTE: completion_boundary={pol['completion_boundary']} leaves "
