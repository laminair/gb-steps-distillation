...
WHY THIS EXISTS. A shared filesystem backed by tape-migrated GPFS is a common shape for
these clusters. A migrated file is fully readable -- `open()` succeeds, the bytes are
correct -- and reading it blocks on a recall that can run to tens of GB. So every cheap
gate passes and the expensive one pays: confirmed directly, a teacher checkpoint cleared
a tokenizer-load gate, a config-key gate and a dry-run gate, because all three ask
questions a migrated file answers correctly. Its nine shards were symlinks into another
account's archive on a shared fileset, eight of them with ONE 512-byte block allocated
against ~5 GB logical each: ~44 GB of tape recall away. The first read would have been
the trainer's, with the GPUs already held, in the PREEMPTABLE queue, killable before
step 1 and paying the recall again on requeue.
...
def weight_files(d: Path) -> list[Path]:
    ...
    granite checkpoints on this filesystem are git-lfs clones, so each carries a `.git` holding a
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        nargs="*",
        metavar="ROLE=PATH",
        help="e.g. student=/path/to/model teacher=/path/to/other; an empty PATH is skipped",
    )
    ap.add_argument(
        "--allow-offline",
        action="store_true",
